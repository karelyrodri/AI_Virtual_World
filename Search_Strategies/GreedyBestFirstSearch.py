...
class GreedyBestFirstSearch():
    ...
    def search(self, current_depth, current_schedule):
        ...
        if (current_depth == self.depth_bound):
            ...
            idx = 0
            while (idx < len(self.actions.actions_list) and frontier_partial_schedules.size() < self.frontier_max_size):  # only considering max frontier size amount of partial schedules
                action = self.actions.actions_list[idx]
                action_node = None
